...
namespace DWL {
	...
	*/
	HWND DTreeView::Crear(DBaseWnd *nPadre, const int cX, const int cY, const int cAncho, const int cAlto, const UINT cID, DWORD nEstilos, DWORD nEstilosExtendidos) {
		_hWnd = CreateWindowEx(nEstilosExtendidos, WC_TREEVIEW, NULL, nEstilos, cX, cY, cAncho, cAlto, nPadre->hWnd(), reinterpret_cast<HMENU>(IntToPtr(cID)), GetModuleHandle(NULL), this);
	...
	*/
	HWND DTreeView::Asignar(DBaseWnd *nPadre, const UINT cID) {
...
//		_Evento_Mouse_BotonSoltado(Boton, ((LPNMITEMACTIVATE)lParam)->ptAction.x, ((LPNMITEMACTIVATE)lParam)->ptAction.y, 0);
		...
		if (DobleClick == false) return SendMessage(GetParent(_hWnd), DWL_TREEVIEW_CLICK, static_cast<WPARAM>(ID()), reinterpret_cast<LPARAM>(&DatosClick));
		else                     return SendMessage(GetParent(_hWnd), DWL_TREEVIEW_DOBLECLICK, static_cast<WPARAM>(ID()), reinterpret_cast<LPARAM>(&DatosClick));
	}
